...
int main(int argc, char** argv) {
    ...
    if (argc == 5) {
        ...
        total_points = std::stoi(argv[3]);
        batchQ = std::stoi(argv[4]);
    }

    int points_per_cluster = total_points / num_clusters;
    ...
    std::cout << "Batch GPU-only STEP synthetic (SoA + warp-reduce)\n";
    std::cout << "dim=" << dim
    ...
    std::vector<float> h_packed_points_aos;
    ...
    pack_points_by_cluster(h_points, total_points, dim, num_clusters, labels, h_packed_points_aos, h_offsets, h_sizes);
    ...
    std::vector<float> h_packed_points_soa((size_t)dim * (size_t)total_points);
    for (int i = 0; i < total_points; ++i) {
        const float* src = h_packed_points_aos.data() + (size_t)i * (size_t)dim;
        for (int j = 0; j < dim; ++j) {
            h_packed_points_soa[(size_t)j * (size_t)total_points + (size_t)i] = src[j];
        }
    }

    float* d_points_soa = nullptr;
    ...
    CUDA_CHECK(cudaMalloc(&d_points_soa, (size_t)dim * (size_t)total_points * sizeof(float)));
    ...
    CUDA_CHECK(cudaMemcpy(d_points_soa, h_packed_points_soa.data(),
                          (size_t)dim * (size_t)total_points * sizeof(float), cudaMemcpyHostToDevice));
    ...
    kernel_dist2qc_and_seed<<<batchQ, blockAB, shAB>>>(d_queries, d_centroids, d_dist2qc, d_seed,
                                                       batchQ, num_clusters, dim);
    CUDA_CHECK(cudaGetLastError());

    int blockC = 128;
    kernel_dk_from_seed_warpreduce_soa<MAX_K><<<batchQ, blockC>>>(d_queries, d_points_soa, d_offsets, d_sizes,
                                                                  d_seed, d_dk, batchQ, dim, k, total_points);
    CUDA_CHECK(cudaGetLastError());

    int blockD = 256;
    kernel_build_active_from_dist2<MAX_ACTIVE><<<batchQ, blockD>>>(d_dist2qc, d_radii, d_dk,
                                                                   d_active_counts, d_active_ids,
                                                                   batchQ, num_clusters);
    CUDA_CHECK(cudaGetLastError());

    int blockE = 128;
    kernel_topk_from_active_warpreduce_soa<MAX_ACTIVE, MAX_K><<<batchQ, blockE>>>(
        d_queries, d_points_soa, d_offsets, d_sizes,
        d_active_counts, d_active_ids,
        d_out_idx, d_out_dist,
        batchQ, dim, k, total_points
    );
    CUDA_CHECK(cudaGetLastError());
    ...
    std::cout << "GPU-only batch pipeline time: "
              << std::fixed << std::setprecision(3) << ms_since(t0, t1) << " ms\n";
    ...
    std::cout << "Active clusters: avg=" << std::fixed << std::setprecision(2)
              << (double)sum_active / (double)batchQ
    ...
    for (int i = 0; i < std::min(5, k); ++i) {
        std::cout << "  #" << i << " idx=" << h_idx[i]
                  << " dist=" << std::fixed << std::setprecision(6) << h_dist[i] << "\n";
    }

    CUDA_CHECK(cudaFree(d_points_soa));
